...
@Recorder
public class MinecraftRecorder {
    public void installBatchSpanProcessorForOtlp(MinecraftExporterConfig.MinecraftExporterRuntimeConfig runtimeConfig,
        ...
        if (runtimeConfig.endpoint.isPresent() && runtimeConfig.endpoint.get().trim().length() > 0) {
            try {
                ...
                MinecraftLateBoundBatchSpanProcessor delayedProcessor = CDI.current()
                        .select(MinecraftLateBoundBatchSpanProcessor.class, Any.Literal.INSTANCE).get();
